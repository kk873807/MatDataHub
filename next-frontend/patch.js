...
const old_lucide = `BookOpen, User, ShieldAlert, Crown, Shield, Menu, X, FileText, HelpCircle, MessageSquare, ChevronDown } from "lucide-react";`;
const new_lucide = `BookOpen, User, ShieldAlert, Crown, Shield, Menu, X, FileText, HelpCircle, MessageSquare, ChevronDown, Settings, CreditCard, LifeBuoy, Keyboard, LogOut } from "lucide-react";`;
code = code.replace(old_lucide, new_lucide);

// 2. Add handleLogout function
const target_anchor = `const tierLabel = userInfo?.is_admin ? "Admin" : (userInfo?.tier || "free").charAt(0).toUpperCase() + (userInfo?.tier || "free").slice(1);`;
const new_anchor = target_anchor + `\n\n  const handleLogout = () => {\n    localStorage.removeItem("token");\n    window.location.href = "/";\n  };\n`;
if (!code.includes('const handleLogout')) {
    code = code.replace(target_anchor, new_anchor);
}

// 3. Remove Resources widget
const old_resources = `<Link href="/resources" className="p-2 rounded-full text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors" title="Resources"><BookOpen className="w-5 h-5" /></Link>`;
code = code.replace(old_resources, '');

// 4. Replace Account link with dropdown
const pattern = /\{isLanding \? \([\s\S]*?<\/Link>\s*\)\s*:\s*\([\s\S]*?<\/Link>\s*\)\}/;

const new_block = `{isLanding ? (
...
code = code.replace(pattern, new_block);

fs.writeFileSync('src/components/TopNav.tsx', code, 'utf-8');
