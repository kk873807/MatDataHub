...
export function InteractiveFeatures() {
  ...
  useEffect(() => {
    ...
    }, 8000); // Slower rotation to allow reading
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="text-center mb-16">
          ...
          <p className="text-slate-600 dark:text-slate-400 max-w-3xl mx-auto text-lg">
            MatDataHub is packed with specialized tools designed to streamline your material selection, structural calculations, and compliance reporting.
          </p>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 items-start">
          ...
          {/* Interactive Tools Breakdown Panel */}
          <div className="w-full lg:w-2/3">
            <div className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl overflow-hidden relative flex flex-col min-h-[600px]">
              
              {/* Header */}
              <div className="bg-slate-100 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 p-6 flex flex-col shrink-0">
                <AnimatePresence mode="wait">
                  {features.map(feat => feat.id === activeTab && (
                    <motion.div 
                      key={feat.id}
                    ...
                    >
                       ...
                       <div>
                         <h3 className="text-2xl font-bold text-slate-900 dark:text-white font-heading">{feat.title} Tools</h3>
                         <p className="text-slate-500 dark:text-slate-400 mt-1">{feat.desc}</p>
                       </div>
                    </motion.div>
                  ))}
                </AnimatePresence>
              ...
              {/* Dynamic Content: Tool Cards */}
              <div className="flex-1 relative bg-slate-50 dark:bg-slate-900/50 p-6 overflow-y-auto">
                <AnimatePresence mode="wait">
                  {features.map(feat => feat.id === activeTab && (
                    <motion.div
                      key={feat.id}
                    ...
                    >
                      {feat.tools.map((tool, i) => (
                        <div key={i} className="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md transition-shadow group flex flex-col h-full">
                           <div className="flex items-center gap-3 mb-3">
                             <div className="p-2 rounded-lg bg-slate-100 dark:bg-slate-900 text-slate-700 dark:text-slate-300 group-hover:bg-blue-100 dark:group-hover:bg-blue-900/40 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                           ...
                           <div className="mt-auto pt-3 border-t border-slate-100 dark:border-slate-700">
                             <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                               <span className="font-bold text-slate-700 dark:text-slate-400 block mb-1">How it's used:</span>
                               {tool.use}
                             </p>
                           </div>
                        </div>
