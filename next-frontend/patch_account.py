...
pattern = r'<Link href="/account" className="flex items-center gap-2 pl-2 pr-3 py-1\.5 rounded-full border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">.*?<span className={inline-flex items-center gap-1 px-2 py-0\.5 rounded-full text-\[10px\] font-bold uppercase tracking-wider border \$\{tierColor\}}>\s*\{tierLabel\}\s*</span>\s*</Link>'
...
code = re.sub(pattern, new_account, code, flags=re.DOTALL)

with open('src/components/TopNav.tsx', 'w', encoding='utf-8') as f:
    f.write(code)

print("Replaced account link via regex")
