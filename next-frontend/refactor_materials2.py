                    ...
                    {sortedMaterials.map((mat, i) => (
                      <tr key={mat.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors group">
                        <td className="px-6 py-4">
                          <Link href={`/materials/${mat.id}`} className="font-bold text-slate-900 dark:text-white group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors text-base">
                            {mat.name}
                          </Link>
                        </td>
                        <td className="px-6 py-4">
                          <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold ${mat.category === "Metal" ? "bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 border border-blue-200 dark:border-blue-800/50" : mat.category === "Polymer" ? "bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400 border border-purple-200 dark:border-purple-800/50" : mat.category === "Ceramic" ? "bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-400 border border-orange-200 dark:border-orange-800/50" : "bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800/50"}`}>
                            {mat.category}
                          </span>
                        </td>
                    ...
                    <div className="space-y-3.5 text-sm mt-auto relative z-10 bg-slate-50 dark:bg-slate-950 p-4 rounded-xl border border-slate-200/60 dark:border-slate-800/60">
                      ...
                      <div>
                        <div className="flex justify-between items-center mb-1.5">
                          ...
                          <span className="text-slate-900 dark:text-white font-bold">{mat.density || '-'} <span className="text-slate-500 dark:text-slate-400 text-xs font-normal">g/cm³</span></span>
                        </div>
                        <div className="w-full bg-slate-200 dark:bg-slate-800 h-1.5 rounded-full overflow-hidden">
