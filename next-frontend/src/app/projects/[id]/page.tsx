    ...
    if (restrictedTools.includes(activeTool) && profile?.tier !== "advanced" && !profile?.is_admin) {
      return (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-12 text-center space-y-4">
          ...
          <Link href="/account" className="inline-block bg-blue-600 hover:bg-blue-500 text-white font-bold px-6 py-2.5 rounded-2xl text-sm transition-all">
            Upgrade to Advanced
          </Link>
        </div>
      );
      ...
      return (
        <div className="space-y-6">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6">
            <h3 className="text-lg font-bold text-slate-900 dark:text-white font-heading mb-4">Add Part to Assembly</h3>
            <form onSubmit={handleAddPart} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
              <div>
                <label className="block text-xs text-slate-500 dark:text-slate-400 mb-1 font-semibold">Part Name</label>
                <input 
                  type="text" 
                  value={partName} 
              ...
              <div className="relative" ref={searchRef}>
                <label className="block text-xs text-slate-500 dark:text-slate-400 mb-1 font-semibold">Material</label>
                <input 
                  type="text" 
                ...
                {searchMatOpen && filteredMats.length > 0 && (
                  <div className="absolute z-50 w-full mt-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-2xl max-h-48 overflow-y-auto">
                    {filteredMats.map(m => (
                      <div 
                        key={m.id} 
          ...
          <div className="space-y-6">
            <div className="p-4 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl flex items-center justify-between">
              ...
              <div className="text-right">
                ...
                <p className="text-lg font-bold text-slate-900 dark:text-white">{selectedItem.mat?.yield_strength_min} MPa</p>
              </div>
            </div>
    ...
    <main className="flex flex-col p-0 w-full h-full overflow-hidden bg-slate-50 dark:bg-slate-950 relative">
      ...
      <div className="h-16 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex items-center justify-between px-6 shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="font-bold text-slate-900 dark:text-white font-heading leading-tight">{project.name}</h1>
            <p className="text-xs text-slate-500 dark:text-slate-400">Workspace IDE</p>
          </div>
        </div>
    ...
    <div class="footer">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-64 border-r border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 p-4 flex flex-col gap-2 overflow-y-auto shrink-0">
          ...
          <button onClick={()=>setActiveTool("cost")} className={`flex items-center gap-3 px-3 py-2.5 rounded-2xl text-sm font-medium transition-colors ${activeTool === 'cost' ? 'bg-green-600/20 text-green-700 dark:text-green-400 border border-green-500/30' : 'text-slate-500 dark:text-slate-400 hover:bg-white dark:hover:bg-slate-900 hover:text-slate-900 dark:hover:text-white border border-transparent'}`}>
