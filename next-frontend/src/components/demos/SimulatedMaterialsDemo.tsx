...
const STEPS: Step[] = [
  // === MATERIALS LIST VIEW ===
  { label: "Materials page — browse 1000+ verified engineering materials", cursor: { x: 240, y: 60 }, duration: 4000, scroll: 0 },
  { label: "Material cards show name, category, key properties & cost at a glance", cursor: { x: 130, y: 150 }, duration: 4500, scroll: 0 },
  { label: "Each card shows Density, Tensile Strength, Thermal K & Cost/kg", cursor: { x: 370, y: 150 }, duration: 4500, scroll: 0 },
  { label: "Toggle between Grid view and Table view", cursor: { x: 420, y: 60 }, duration: 3500, click: true, scroll: 0 },
  { label: "Use Category filter: Metal, Polymer, Ceramic, Composite", cursor: { x: 100, y: 95 }, duration: 4000, click: true, scroll: 0 },
  { label: "Sort by Name, Cost, Tensile Strength, or Density", cursor: { x: 350, y: 95 }, duration: 3500, click: true, scroll: 0 },

  // === SEARCH FLOW ===
  { label: "Search by material name, grade, or standard...", cursor: { x: 200, y: 55 }, duration: 3000, click: true, scroll: 0 },
  { label: "Typing 'Gold'... autocomplete suggestions appear", cursor: { x: 200, y: 55 }, duration: 3000, scroll: 0 },
  { label: "Click a result to open its full detail view", cursor: { x: 200, y: 140 }, duration: 2500, click: true, scroll: 0 },

  // === DETAIL VIEW ===
  { label: "Material header: category badge, name, description & applications", cursor: { x: 240, y: 80 }, duration: 5000, scroll: 0 },
  { label: "'Add to Compare' — send to radar chart analysis", cursor: { x: 80, y: 168 }, duration: 3500, click: true, scroll: 0 },
  { label: "'Find Substitution' — AI multi-objective alternative finder", cursor: { x: 190, y: 168 }, duration: 3500, click: true, scroll: 0 },
  { label: "'Use in Synthesizer' — blend as matrix or reinforcement", cursor: { x: 300, y: 168 }, duration: 3500, click: true, scroll: 0 },
  { label: "'Export PDF' — download full material datasheet", cursor: { x: 400, y: 168 }, duration: 3000, click: true, scroll: 0 },
  
  // SCROLL DOWN
  { label: "Market Price — real-time cost with trend indicator", cursor: { x: 130, y: 225 }, duration: 4500, scroll: -60 },
  { label: "Standards & Identifiers — ASTM/ISO, Grade, Equivalents, Data Source", cursor: { x: 370, y: 225 }, duration: 5000, scroll: -60 },
  
  // SCROLL DOWN MORE
  { label: "Full properties: Density, Yield, UTS, Modulus, Thermal K, Hardness", cursor: { x: 240, y: 305 }, duration: 5000, scroll: -120 },

  // === HISTORICAL PRICE CHART ===
  { label: "Historical Price Tracking (12M) — SVG area chart showing cost trends over time", cursor: { x: 240, y: 380 }, duration: 6000, scroll: -220 },

  // === SIMILAR MATERIALS & FILTERS ===
  { label: "Similar Materials — discover related alloys and alternatives", cursor: { x: 240, y: 445 }, duration: 4500, scroll: -300 },
  { label: "Advanced Filters — set min tensile, max cost, min thermal conductivity", cursor: { x: 130, y: 480 }, duration: 4500, click: true, scroll: -300 },
  { label: "Export material datasheet to CSV or share via link", cursor: { x: 370, y: 480 }, duration: 3500, click: true, scroll: -300 },

  // Reset
  { label: "", cursor: { x: 460, y: 500 }, duration: 1000, scroll: 0 },
];
...
export function SimulatedMaterialsDemo() {
  ...
  const showDetail = step >= 9 && step <= 21;
  ...
  return (
    <div className="w-full bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-4 sm:p-6 relative">
      ...
      {/* FIXED HEIGHT WINDOW (The screen recording box) */}
      <div className="relative z-10 w-full max-w-lg mx-auto bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl h-[420px] overflow-hidden flex flex-col">
        ...
        <div className="px-5 py-3 border-b border-slate-100 dark:border-slate-800 flex items-center gap-2 bg-white dark:bg-slate-950 z-20 shrink-0 shadow-sm">
          <Database className="w-4 h-4 text-blue-500" />
          <span className="text-sm font-bold text-slate-900 dark:text-white">Material Database</span>
        </div>

        {/* SCROLLING CONTENT - Animated via Framer Motion y-offset */}
        <motion.div 
          className="p-5 flex-1 relative w-full h-full"
        ...
        >
          <AnimatePresence mode="wait">
            ...
            {showDetail && (
              <motion.div key="detail" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} className="space-y-3 pb-8">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className={`p-3 rounded-xl border transition-all ${step === 14 ? "border-emerald-400 ring-2 ring-emerald-200 shadow-md bg-white dark:bg-slate-900" : "border-slate-200 dark:border-slate-700"}`}>
                  ...
                  <div className={`p-3 rounded-xl border transition-all ${step === 15 ? "border-violet-400 ring-2 ring-violet-200 shadow-md bg-white dark:bg-slate-900" : "border-slate-200 dark:border-slate-700"}`}>
                ...
                {/* Properties */}
                <div className={`rounded-xl border p-3 transition-all ${step === 16 ? "border-blue-400 ring-2 ring-blue-200 shadow-md bg-white dark:bg-slate-900" : "border-slate-200 dark:border-slate-700"}`}>
                ...
                {/* Historical Price Chart */}
                <div className={`rounded-xl border p-3 transition-all ${step === 17 ? "border-emerald-400 ring-2 ring-emerald-200 shadow-lg bg-white dark:bg-slate-900" : "border-slate-200 dark:border-slate-700"}`}>
                ...
                {/* Similar Materials */}
                <div className={`rounded-xl border p-3 transition-all ${step === 18 ? "border-amber-400 ring-2 ring-amber-200 shadow-md bg-white dark:bg-slate-900" : "border-slate-200 dark:border-slate-700"}`}>
                ...
                <div className="flex gap-2">
                  <div className={`flex-1 p-2.5 rounded-xl border text-center transition-all ${step === 19 ? "border-blue-400 ring-2 ring-blue-200 shadow-md bg-white dark:bg-slate-900" : "border-slate-200 dark:border-slate-700"}`}>
                  ...
                  <div className={`flex-1 p-2.5 rounded-xl border text-center transition-all ${step === 20 ? "border-amber-400 ring-2 ring-amber-200 shadow-md bg-white dark:bg-slate-900" : "border-slate-200 dark:border-slate-700"}`}>
          ...
          {/* THE CURSOR - Absolutely positioned inside the moving container */}
