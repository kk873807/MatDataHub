...
export function BeamDeflection() {
  ...
  const handleCalculate = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API}/calculators/beam_deflection`, {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xl w-full h-full flex flex-col">
      ...
      <button onClick={handleCalculate} disabled={loading} className="mt-6 w-full py-2.5 rounded-2xl bg-purple-600 hover:bg-purple-700 text-white font-medium flex justify-center items-center">
