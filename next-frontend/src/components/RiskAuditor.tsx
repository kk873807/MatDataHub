          ...
          {result ? (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="h-full flex flex-col justify-between">
              <div className="p-4 rounded-xl border" style={{ borderColor: result.geopolitics.risk_color, backgroundColor: `${result.geopolitics.risk_color}15` }}>
                ...
                <p className="text-sm text-slate-600 dark:text-slate-300">{result.geopolitics.risk_text.replace(/\*\*/g, '')}</p>
              </div>
             ...
             <div className="h-full flex items-center justify-center border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-xl text-slate-600 dark:text-slate-300 text-sm">
